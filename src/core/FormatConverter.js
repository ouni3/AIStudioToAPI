...
class FormatConverter {
    ...
    translateGoogleToOpenAIStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        if (candidate.finishReason) {
            ...
            if (candidate.finishReason === "SAFETY") {
                ...
                // Ensure delta.content is emitted if we never emitted any text but role has been sent or thinking occurred.
                // If model only generated reasoning/thinking without content, provide mock thinking complete response.
                if (!streamState.contentSent && !streamState.hasFunctionCall) {
                    const fallbackContent = streamState.thinkingSent
                        ? FormatConverter.MOCK_EMPTY_THINKING_RESPONSE
                        : "";
                    const fallbackDelta = { content: fallbackContent };
                    if (!streamState.roleSent) {
    ...
    translateGoogleToResponseAPIStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        const handleGoogleResponseObject = googleResponse => {
            ...
            if (candidate.finishReason && !streamState.completed) {
                ...
                // provide a mock output message item.
                if (
                    !streamState.messageItem &&
                    streamState.reasoningItem &&
                    ...
                    const messageItem = ensureMessageItem();
                    const note = FormatConverter.MOCK_EMPTY_THINKING_RESPONSE;
                    streamState.messageText += note;
                    pushEvent("response.output_text.delta", {
                        content_index: messageItem.content_index,
                        delta: note,
    ...
    convertGoogleToOpenAINonStream(googleResponse, modelName = "gemini-2.5-flash-lite") {
        ...
        // provide mock thinking complete response to avoid empty content for downstream clients
        const hasThoughts =
        ...
        if (!content && tool_calls.length === 0 && hasThoughts) {
            content = FormatConverter.MOCK_EMPTY_THINKING_RESPONSE;
        }
    ...
    convertGoogleToResponseAPINonStream(googleResponse, modelName = "gemini-2.5-flash-lite", responseDefaults = {}) {
        ...
        // Add message output if present or if model only reasoned without content
        if (!messageContent && reasoningContent && output.filter(o => o.type === "function_call").length === 0) {
            messageContent = FormatConverter.MOCK_EMPTY_THINKING_RESPONSE;
        }
    ...
    translateGoogleToClaudeStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        if (candidate.finishReason) {
            ...
            if (candidate.finishReason === "SAFETY") {
                ...
                // If model only had thinking block or was completely empty, emit a valid text block
                const fallbackText = streamState.thinkingBlockStarted
                    ? FormatConverter.MOCK_EMPTY_THINKING_RESPONSE
                    : "";
                events.push({
                    content_block: { text: fallbackText, type: "text" },
                    index: streamState.contentBlockIndex,
                    type: "content_block_start",
                });
                events.push({
                    delta: { text: fallbackText, type: "text_delta" },
                    index: streamState.contentBlockIndex,
                    type: "content_block_delta",
                });
                events.push({
    ...
    convertGoogleToClaudeNonStream(googleResponse, modelName = "gemini-2.5-flash-lite") {
        ...
        // provide mock thinking complete text block so client receives a readable message.
