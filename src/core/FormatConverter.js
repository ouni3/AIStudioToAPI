...
class FormatConverter {
    ...
    static MOCK_EMPTY_THINKING_RESPONSE = "[Thinking completed, but no text response was generated by the model.]";
    static FALLBACK_GLOB_TOOL_NAME = "glob";
    static FALLBACK_GLOB_ARGS_JSON = JSON.stringify({ pattern: "*" });
    static FALLBACK_GLOB_ARGS_OBJ = { pattern: "*" };
    ...
    translateGoogleToOpenAIStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        if (googleResponse.usageMetadata) {
            ...
            if (
                googleResponse.usageMetadata.thoughtsTokenCount > 0 ||
                (streamState.usage?.completion_tokens_details &&
                    streamState.usage.completion_tokens_details.reasoning_tokens > 0)
            ) {
                streamState.thinkingSent = true;
            }
        }
            ...
            for (const part of candidate.content.parts) {
                ...
                if (part.thought === true) {
                    ...
                    if (typeof part.text === "string" && part.text.trim() !== "") {
                        streamState.hasEffectiveContent = true;
                    }
        ...
        if (candidate.finishReason) {
            ...
            if (candidate.finishReason === "SAFETY") {
            ...
            } else if (streamState.thinkingSent && !streamState.contentSent && !streamState.hasFunctionCall) {
                // Thinking-only case: inject harmless glob tool_call fallback so client (e.g. Kilocode) continues execution
                const fallbackToolCallId = `call_fallback_glob_${this._generateRequestId()}`;
                const toolCallIndex = streamState.toolCallIndex ?? 0;
                streamState.toolCallIndex = toolCallIndex + 1;
                const fallbackToolCallObj = {
                    function: {
                        arguments: FormatConverter.FALLBACK_GLOB_ARGS_JSON,
                        name: FormatConverter.FALLBACK_GLOB_TOOL_NAME,
                    },
                ...
                const fallbackDelta = {
                    tool_calls: [fallbackToolCallObj],
                };
                if (!streamState.roleSent) {
                ...
                // Ensure delta.content is emitted if we never emitted any text but role has been sent or empty response.
    ...
    /**
     * Finalize OpenAI stream when connection ends unexpectedly or finishes without finishReason.
     * If thinking was sent but no effective content or function call occurred, inject harmless glob tool call.
     * @param {object} streamState - Stream state object
     * @param {string} modelName - Model name
     * @param {string} streamId - Stream ID
     * @returns {string|null} - SSE formatted fallback chunks or null
     */
    finalizeOpenAIStream(streamState, modelName = "gemini-2.5-flash-lite", streamId = null) {
        if (!streamState) return null;
        if (streamState.fallbackEmitted) return null;

        const effectiveStreamId = streamId || streamState.id || `chatcmpl-${this._generateRequestId()}`;
        const created = streamState.created || Math.floor(Date.now() / 1000);

        if (
            !streamState.hasFunctionCall &&
            streamState.thinkingSent &&
            (!streamState.contentSent || !streamState.hasEffectiveContent)
        ) {
            const fallbackToolCallId = `call_fallback_glob_${this._generateRequestId()}`;
            const toolCallIndex = streamState.toolCallIndex ?? 0;
            streamState.toolCallIndex = toolCallIndex + 1;
            ...
            if (streamState.usage) {
                chunk2.usage = streamState.usage;
            }

            streamState.hasFunctionCall = true;
            streamState.fallbackEmitted = true;

            return `data: ${JSON.stringify(chunk1)}\n\ndata: ${JSON.stringify(chunk2)}\n\n`;
        }
    ...
    translateGoogleToResponseAPIStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        const handleGoogleResponseObject = googleResponse => {
            ...
            if (candidate.finishReason && !streamState.completed) {
                ...
                // If stream only had reasoning output items without message item or tool calls,
                // inject harmless glob tool_call fallback so client (e.g. Kilocode) continues execution
                if (
                    !streamState.messageItem &&
                    streamState.reasoningItem &&
                    streamState.outputItemsByIndex.filter(item => item?.type === "function_call").length === 0
                ) {
                    const fallbackItemId = `fc_${this._generateRequestId()}`;
                    const fallbackCallId = `call_fallback_glob_${this._generateRequestId()}`;
                    const fallbackOutputIndex = streamState.nextOutputIndex++;
                    const fallbackArgs = FormatConverter.FALLBACK_GLOB_ARGS_JSON;
                    const fallbackToolName = FormatConverter.FALLBACK_GLOB_TOOL_NAME;
                    ...
                    pushEvent("response.output_item.added", {
                        ...
                        output_index: fallbackOutputIndex,
                    });
                    ...
                    streamState.outputItemsByIndex[fallbackOutputIndex] = completedFallbackItem;

                    pushEvent("response.output_item.done", {
                        item: completedFallbackItem,
                        output_index: fallbackOutputIndex,
                    });
                }
    ...
    convertGoogleToOpenAINonStream(googleResponse, modelName = "gemini-2.5-flash-lite") {
        ...
        // If candidate only had thinking/reasoning parts or thoughtsTokenCount without any text content or tool calls,
        // inject harmless glob tool_call fallback so client (e.g. Kilocode) continues execution
        const hasThoughts =
            reasoning_content || (googleResponse.usageMetadata && googleResponse.usageMetadata.thoughtsTokenCount > 0);
        if (!content && tool_calls.length === 0 && hasThoughts) {
            const fallbackToolCallId = `call_fallback_glob_${this._generateRequestId()}`;
            tool_calls.push({
                function: {
                    arguments: FormatConverter.FALLBACK_GLOB_ARGS_JSON,
        ...
        const message = { content: content !== undefined ? content : "", role: "assistant" };
    ...
    convertGoogleToResponseAPINonStream(googleResponse, modelName = "gemini-2.5-flash-lite", responseDefaults = {}) {
        ...
        // Add message output if present or inject fallback glob function_call if model only reasoned without content
        if (!messageContent && reasoningContent && output.filter(o => o.type === "function_call").length === 0) {
            const fallbackCallId = `call_fallback_glob_${this._generateRequestId()}`;
            output.push({
                arguments: FormatConverter.FALLBACK_GLOB_ARGS_JSON,
                call_id: fallbackCallId,
    ...
    translateGoogleToClaudeStream(googleChunk, modelName = "gemini-2.5-flash-lite", streamState = null) {
        ...
        if (candidate.finishReason) {
            ...
            } else if (!streamState.textBlockStarted && !streamState.hasToolUse) {
                if (streamState.thinkingBlockStarted) {
                    // Thinking-only case: inject harmless glob tool_use fallback so client (e.g. Kilocode) continues execution
                    const toolUseId = `toolu_fallback_glob_${this._generateRequestId()}`;
                    events.push({
                        content_block: {
                            id: toolUseId,
                            input: {},
                    ...
                    events.push({
                        delta: {
                            partial_json: FormatConverter.FALLBACK_GLOB_ARGS_JSON,
                            type: "input_json_delta",
                        },
                        index: streamState.contentBlockIndex,
                        type: "content_block_delta",
    ...
    /**
     * Finalize Claude stream when connection ends unexpectedly or finishes without finishReason.
     * If thinking was started but no text or tool use occurred, inject harmless glob tool use.
     * @param {object} streamState - Stream state object
     * @param {string} modelName - Model name
     * @param {string} messageId - Message ID
     * @returns {string|null} - SSE formatted events or null
     */
    finalizeClaudeStream(streamState, modelName = "gemini-2.5-flash-lite", messageId = null) {
        if (!streamState) return null;
        if (streamState.fallbackEmitted) return null;

        const effectiveMessageId = messageId || streamState.messageId || `msg_${this._generateRequestId()}`;
        if (!streamState.messageId) {
            streamState.messageId = effectiveMessageId;
        }
        ...
        if (!streamState.hasToolUse && streamState.thinkingBlockStarted && !streamState.textBlockStarted) {
            if (!streamState.thinkingBlockStopped) {
                events.push({
                    index: streamState.thinkingBlockIndex,
                    type: "content_block_stop",
                });
    ...
    convertGoogleToClaudeNonStream(googleResponse, modelName = "gemini-2.5-flash-lite") {
        ...
        // If content contains only thinking blocks without any text blocks or tool_use,
        // inject harmless glob tool_use fallback so client (e.g. Kilocode) continues execution
        const hasTextOrTool = content.some(item => item.type === "text" || item.type === "tool_use");
        if (content.length > 0 && !hasTextOrTool) {
            content.push({
                id: `toolu_fallback_glob_${this._generateRequestId()}`,
                input: FormatConverter.FALLBACK_GLOB_ARGS_OBJ,
                name: FormatConverter.FALLBACK_GLOB_TOOL_NAME,
                type: "tool_use",
            });
            stopReason = "tool_use";
