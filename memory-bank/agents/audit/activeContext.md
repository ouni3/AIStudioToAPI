...
## 1. 核心经验条目 (Max 8)
1. **API / 网关 Caller DX 审查标准**: 重点关注调用心智、响应延迟降维及错误提示亲和度，确保畸形入参（如 "-"、剥离后缀后非法）在协议入口毫秒级拦截 (≤30ms) 并返回标准 400 Bad Request。
2. **404 优先中断与防雪崩切号机制**: 上游模型不存在时直接阻断回显 404，绝不盲目触发换号重试，彻底杜绝凭据池雪崩消耗与在途队列积压。
